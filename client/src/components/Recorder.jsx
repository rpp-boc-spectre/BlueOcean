...
export default function RecorderTone({ currentList, setAudioLayers }) {
  ...
  const handleUploadClick = async () => {
    try {
      const mp3StorageRef = ref(
        storage,
        `audio/${user.uid}/${recordingName}.webm`
      );
      await uploadBytes(mp3StorageRef, micRecorder);
      snackbar.showMessage(
        <Alert
          severity='success'
          sx={{ width: '100%' }}>{`Layer Uploaded :)`}</Alert>
      );
      ...
      snackbar.showMessage(
        <Alert
          ...
          sx={{
            width: '100%',
          }}>{`There was an error uploading your layer :(`}</Alert>
      );
    }
  };
  ...
  useEffect(() => {
    return () => {
      ...
      if (
        micRecorderRef.current !== null &&
        !(micRecorderRef.current instanceof Blob) &&
        micRecorderRef.current !== undefined
      ) {
        micRecorderRef.current.dispose();
      }
      // close mic on stop.
      if (userMicRef.current !== null &&  userMicRef.current !== undefined) {
  ...
  return (
    <>
      <Switch
        checked={playWith}
        onChange={() => {
          setPlayWith((prev) => !prev);
        }}
      />
      {!!micRecorder && !isFinished && (
        <Typography>{`${Math.abs(
          timeRemaining - recordingLimit.current
        ).toFixed(2)} / ${recordingLimit.current}:00`}</Typography>
      )}
      {countDown && (
        <Typography>{(currentCountDown / 1000).toFixed(2)}</Typography>
      )}
      <Button
        variant='outlined'
      ...
      <ValidatorForm onSubmit={handleUploadClick}>
        ...
        />
        <Button variant='contained' type='submit' disabled={!isFinished}>
          Upload
        </Button>
      </ValidatorForm>
      <br />
